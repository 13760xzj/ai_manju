...
import type { TabOption } from "./TabSelector";
import { CustomSelect } from "@/components/common";
import ModalSelect from "./ModalSelect";
import TabSelector from "./TabSelector";
import AgentSelect from "./AgentSelect";

const ScriptParam: React.FC = () => {
  const agentOptions = [
    {
      value: "manhua_v3",
      label: "分镜编剧（漫剧版v3）",
  ...
  const tabOptions: Array<TabOption> = [
    {
      id: "compact",
      label: "精简模式",
      description: "镜头少而精，强调核心情节（约 35 镜头/千字剧本）",
    },
    {
      id: "standard",
      label: "标准模式",
      description: "结构完整，情节与细节兼顾（约 65 镜头/千字剧本）",
    },
    {
      id: "detailed",
      label: "细拆模式",
      description: "特写与动作拆解，画面感强（约 80 镜头/千字剧本）",
    },
  ];
  ...
  const [selectedTab, setSelectedTab] = useState<string>("compact");

  return (
    <div className="w-full h-full">
      <div className="text-xs">分镜脚本</div>
      <div className="mt-3! flex items-center gap-3">
        ...
        <div className="flex-2 ">
          <div className="text-xs ">镜头密度</div>
          <div className="mt-2!">
            <TabSelector
              options={tabOptions}
              defaultValue={selectedTab}
              onChange={(value) => {
                console.log("选中的 Tab:", value);
      ...
      <div className="text-xs mt-3!">分镜图（消耗4/张）</div>
      <div className="text-xs mt-3!">智能体</div>
      <div className="mt-3! flex items-center gap-3">
        <div className="flex-1">
          <div className="text-xs">智能体</div>
          <div className="mt-2!">
