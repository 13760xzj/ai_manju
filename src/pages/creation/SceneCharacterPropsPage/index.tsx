    ...
    <div className="settings-page">
      <div className="tabs">
        ...
        <div style={{ display: 'flex', gap: '10px', marginLeft: 'auto', marginRight: '10px' }}>
          <button className="btn-small btn-success-small">{getRegenerateButtonText()}</button>
                  ...
                  <div className="view-card-header">
                    ...
                    <div className="view-card-menu">
                      <button 
                        ...
                        onClick={(e) => { e.stopPropagation(); toggleDropdown(`scene-${view}`); }}
                      >
                        ⋮
                      </button>
        ...
        <div className="tab-content" onClick={closeDropdown}>
          {[1, 2].map((charNum) => (
            <div key={charNum} className="scene-card">
              ...
              {[1, 2].map((formNum) => (
                <div key={formNum} style={{ marginBottom: '20px', paddingBottom: formNum === 1 ? '20px' : '0', borderBottom: formNum === 1 ? '1px solid #f0f0f0' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: '#333' }}>形态 {formNum}</div>
                    <div className="scene-actions">
                  ...
                  <div className="view-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
                    {['三视图', '全身照'].map((view) => (
                      <div key={view} className="view-card">
                        <div className="view-card-header">
                          <div className="view-title">{view}</div>
                          <div className="view-card-menu">
                            <button 
                              className="view-card-menu-btn"
                              onClick={(e) => { e.stopPropagation(); toggleDropdown(`char-${charNum}-${formNum}-${view}`); }}
                            >
                              ⋮
                            </button>
                            {activeDropdown === `char-${charNum}-${formNum}-${view}` && (
                              <div className="view-card-dropdown">
                                <button 
                                  className="view-card-dropdown-item"
                                  onClick={() => handleDownload('角色图', `角色${charNum}-形态${formNum}-${view}`)}
                                >
                                  📥 下载
                                </button>
                                <button 
                                  className="view-card-dropdown-item danger"
                                  onClick={() => handleDelete('角色图', `角色${charNum}-形态${formNum}-${view}`)}
                                >
                                  🗑️ 删除
                                </button>
                              </div>
          ...
          {[1, 2].map((propNum) => (
            <div key={propNum} className="scene-card">
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#333' }}>形态 1</div>
                  <div className="scene-actions">
                    <button className="btn-mini btn-gray">编辑形态图</button>
                    <button className="btn-mini btn-gray">复制形态</button>
                    <button className="btn-mini btn-red">删除形态</button>
                ...
                <div className="view-grid" style={{ gridTemplateColumns: 'repeat(1, 1fr)', maxWidth: '300px' }}>
                  <div className="view-card">
                    <div className="view-card-header">
                      ...
                      <div className="view-card-menu">
                        <button 
                          className="view-card-menu-btn"
                          onClick={(e) => { e.stopPropagation(); toggleDropdown(`prop-${propNum}`); }}
                        >
                          ⋮
                        </button>
                        {activeDropdown === `prop-${propNum}` && (
                          <div className="view-card-dropdown">
                            <button 
                              className="view-card-dropdown-item"
                              onClick={() => handleDownload('道具图', `道具${propNum}`)}
                            >
                              📥 下载
                            </button>
                            <button 
                              className="view-card-dropdown-item danger"
                              onClick={() => handleDelete('道具图', `道具${propNum}`)}
                            >
                              🗑️ 删除
                            </button>
                          </div>
